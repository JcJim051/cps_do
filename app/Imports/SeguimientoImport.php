...
class SeguimientoImport implements ToCollection, WithHeadingRow, WithValidation, SkipsOnFailure 
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) 
        {
            ...
            try {
                ...
                // ✅ ACTUALIZADO CON anio en la clave
                ...
                $this->logicFailures[] = [
                    'row' => $rowNumber,
                    'cedula' => $cedula,
                    'errors' => ["Error interno al crear/actualizar el seguimiento: " . $e->getMessage()],
                ];
    ...
    public function rules(): array
    {
        return [
            ...
            'fecha_aut_despacho'    => ['nullable', 'date'],
    ...
    public function exportPdf($id)
    {
        ...
        $html = view('vendor.backpack.crud.show_autorizaciones', [
            'entry' => $entry,
            'crud' => $this->crud
        ])->render();

        return \PDF::loadHTML($html)->setPaper('a4')->stream("autorizacion_$id.pdf");
    }
}